...
Client.prototype.requestFormUpload = function (uri, file, callback) {
  var self     = this,
      out,
      auth     = this.options.get('password') ? ':' + this.options.get('password') : '/token:' + this.options.get('token'),
      encoded  = new Buffer(this.options.get('username') + auth).toString('base64'),
  ...
  let submitOptions = url.parse(uri);
  submitOptions.auth = this.options.get('username') + '/token:' + this.options.get('token');
